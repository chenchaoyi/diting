...
# Pack the AppIcon.iconset PNGs (committed under Resources/) into
# a single .icns the bundle can advertise via CFBundleIconFile.
# iconutil ships with macOS — no librsvg / Inkscape / Pillow at
# build time. The PNG sources are regenerated by
# scripts/build_app_icon.py only when the logo changes.
if [ -d Resources/AppIcon.iconset ]; then
    echo "==> iconutil --convert icns"
    iconutil --convert icns \
        Resources/AppIcon.iconset \
        --output "$BUNDLE/Contents/Resources/AppIcon.icns"
else
    echo "warn: Resources/AppIcon.iconset not found; bundle will use" \
         "the generic .app icon" >&2
fi
...
echo
echo "Next steps:"
echo "  1. open $BUNDLE                 # walks Location → Bluetooth → Notifications"
echo "  2. Click Allow on each prompt as it appears (one at a time)"
