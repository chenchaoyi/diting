...
| `--for-llm [outdir]` writes `report.md` + `prompt.txt` bundle; default outdir is `./diting-llm-<timestamp>/` (cwd-relative — tests pass an explicit tmp_path or chdir to tmp_path, never the repo tree) | `test_analyze.py::test_for_llm_writes_report_markdown`, `::test_for_llm_writes_prompt_txt`, `::test_for_llm_default_outdir_is_cwd_relative_not_repo` |
...
| **agent-friendly-cli** — `--json`: `analyze` emits one pure JSON doc (`report_to_dict`, stable English keys even under `--lang zh`); `once` emits `{backend,permission_state,associated,connection}`; `watch` shapes one object per event (`_watch_event_to_json`, deduped); `connection_to_dict` round-trips | `test_cli.py::test_analyze_json_is_pure_parseable_document`, `::test_analyze_json_keys_stay_english_under_zh`, `::test_watch_event_to_json_shapes_each_kind`, `::test_connection_to_dict_round_trips`, `test_analyze.py::test_report_to_dict_is_json_serializable_with_stable_keys` |
| **agent-friendly-cli** — each subcommand accepts `--help` with EXAMPLES + the `--json` note; CLI help is English-only by design (runtime prose / errors stay bilingual) | `test_cli.py::test_subcommand_help_prints_and_exits_zero` |
