...
| **add-insight-events** — a new `insight` event type carries a stable English `code`, a `severity` (`info`/`note`/`warn`), and flattened `detail` keys (omitted when absent); the human one-liner is generated at render time, not serialised | `test_event_log.py::test_emit_insight_writes_code_severity_and_detail`, `::test_emit_insight_omits_detail_when_absent` |
...
| `insight` salience maps from severity: `warn`→`high`, `note`→`notable`, `info`→`low` | `test_event_log.py::test_emit_insight_writes_code_severity_and_detail`, `::test_warn_insight_salience_is_high` |
