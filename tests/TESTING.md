...
| `BLEPoller` emits `BLEDeviceSeenEvent` on first identifier observation; emits `BLEDeviceLeftEvent` on TTL eviction; no debounce, no re-emit on subsequent observations | `test_ble.py::test_poller_emits_seen_event_on_first_observation`, `::test_poller_does_not_re_emit_seen_for_known_identifier`, `::test_poller_emits_left_event_on_ttl_eviction`, `::test_poller_connected_peripheral_does_not_re_emit_seen` |
...
| OUI refresh script parses IEEE MA-L CSV into the canonical JSON shape | `test_lan.py::test_oui_refresh_script_parses_csv_to_aabbcc_keys`, `::test_oui_refresh_script_skips_non_ma_l_rows`, `::test_oui_refresh_script_dedupes_repeated_assignments` |
