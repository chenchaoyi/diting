...
| Classifier: HomePod (`AirPlay` + `AirPlay audio`) → speaker; iPad (`AirPlay` + `Apple Companion`) → phone; Apple TV (AirPlay alone) → tv. **Needles match the human-readable category strings the mdns module stores on each LANHost (`AirPlay`, `AirPlay audio`, `Apple Companion`, `HomeKit`, `Sonos`, …) — NOT the raw service-type names (`_raop._tcp`, `_companion-link._tcp`) which never appear in `bonjour_services`.** | `test_device_class.py::test_homepod_airplay_audio_signals_speaker_not_tv`, `::test_homepod_full_apple_signature_signals_speaker_not_phone`, `::test_ipad_airplay_plus_companion_signals_phone_not_tv`, `::test_apple_tv_airplay_alone_still_signals_tv` |
