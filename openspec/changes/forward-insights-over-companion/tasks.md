...
- [ ] 0.1 Confirm per-envelope minimum-version stamping (recommended) vs a
  flag-day all-v2 bump. (Blocks the wire work.)

## 1. Test plan (tests-first)
- [ ] 1.1 `tests/TESTING.md` (EN) — companion-protocol `insight` type + `obj`
  tag + v2 + per-envelope stamping; companion-bridge insight-forward rows;
  events nested-detail update. Mirror `docs/zh/TESTING.md`.

## 2. Wire shape
- [ ] 2.1 `protocol/_schema_spec.py` — add the `insight` spec + the `obj` tag
  (doc the tag table).
- [ ] 2.2 `protocol/events_schema.py` — `_tag_ok` + `_TAG_SCHEMA`/`_field_schema`
  handle `obj` (`{"type":"object"}`, inner keys unconstrained).
- [ ] 2.3 `event_log.py` — `emit_insight` emits `detail` as a nested object (drop
  the flatten); update the v1.13.0 flattened-shape tests to nested.

## 3. Versioning
- [ ] 3.1 `protocol/version.py` — `PROTOCOL_VERSION = 2`,
  `SUPPORTED_VERSIONS = {1, 2}`; `EVENT_MIN_VERSION = {"insight": 2}`.
- [ ] 3.2 Seal path (`crypto.seal_event` / `sink`) — stamp the envelope `v` at
  the event's min version; envelope schema `v` enum → `[1, 2]`.

## 4. Push wiring
- [ ] 4.1 `push_policy.py` — add `insight` to `DEFAULT_PUSH_TYPES`; `_target`
  insight branch → `code`. (Salience gate already drops `info`.)

## 5. Regenerate + vendor
- [ ] 5.1 `python -m diting.companion.protocol._generate` — regenerate
  `event.schema.json`, `fixtures/events.jsonl` (gains an `insight` line),
  `manifest.json`. Commit the regenerated artifacts.

## 6. Tests
- [ ] 6.1 Protocol: insight validates (nested detail, unknown inner keys ok);
...
## 7. Gates
- [ ] 7.1 `uv run pytest`, snapshot regression, `openspec validate --specs --strict`,
