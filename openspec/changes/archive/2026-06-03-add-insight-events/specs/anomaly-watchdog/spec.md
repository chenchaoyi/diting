...
for `insight` events whose `severity` is `note` or `warn`, using the synthesized
human one-liner as the body, subject to the same per-(type, target) silence
window as the anomaly types — keyed by the insight `code` so distinct insights
debounce independently. `info`-severity insights SHALL NOT notify (they remain
log + TUI only). As with the anomaly types, a missing helper binary SHALL be a
