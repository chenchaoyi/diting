...
### ADDED Requirement: Five event types SHALL share one schema and one ring
...
### ADDED Requirement: Each event SHALL be a frozen dataclass with an explicit timestamp
...
### ADDED Requirement: The `EventRing` SHALL be size-bounded and thread-safe-by-construction
...
### ADDED Requirement: JSONL serialisation SHALL use locale-stable English keys
...
### ADDED Requirement: Timestamps in the JSONL stream SHALL be local-TZ ISO-8601 with offset
...
### ADDED Requirement: `NetworkChangeEvent` SHALL be a control-plane signal, not a user-visible event
