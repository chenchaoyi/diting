...
mode and the headless `wifiscope monitor` subcommand. Both modes
...
### ADDED Requirement: Both `--log` and `wifiscope monitor` SHALL produce byte-identical streams
The `EventLogger` class SHALL be the single point of truth for the
wire format. The TUI's `--log <path>` and the headless `wifiscope
...
- **WHEN** the user runs `wifiscope --log a.jsonl` and `wifiscope monitor > b.jsonl` in the same Wi-Fi environment for the same duration
- **THEN** `diff <(jq -c . a.jsonl) <(jq -c . b.jsonl)` shows differences only in timestamps, never in field shape

### ADDED Requirement: The writer SHALL flush after every event for crash safety
...
- **WHEN** the user runs `kill -9 $(pgrep wifiscope)` after 5 minutes of logging
- **THEN** the JSONL file is complete up to the last `flush()` call (i.e. up to the last event)

### ADDED Requirement: An atexit hook SHALL close the writer cleanly on graceful exit
...
### ADDED Requirement: JSONL keys SHALL be locale-stable English regardless of UI language
The writer SHALL use English JSON keys (`type`, `bssid`, `ssid`,
`state`, `magnitude_db`, etc.) even when `WIFISCOPE_LANG=zh` is set.
...
- **WHEN** the user runs `WIFISCOPE_LANG=zh wifiscope --log /tmp/wifi.jsonl`
- **THEN** roam events lay down as `{"type":"roam","previous_ssid":"咖啡馆", ...}`, NOT escaped to `\\uXXXX` sequences

### ADDED Requirement: Timestamps SHALL be local-TZ ISO-8601 with offset
...
### ADDED Requirement: The writer SHALL accept `None` as a no-op
...
- **WHEN** the user runs `wifiscope` (no log path)
- **THEN** `app._event_logger.emit_roam(...)` calls are no-ops; no file is opened, no IO happens

### ADDED Requirement: Connection-update events SHALL ride alongside the five event types
...
- **WHEN** the user runs `wifiscope --log` and stays on one BSSID for an hour
