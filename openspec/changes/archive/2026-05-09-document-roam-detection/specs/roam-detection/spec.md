...
### ADDED Requirement: The current link SHALL get a 0–100 simple score
...
### ADDED Requirement: Same-SSID better-candidate detection SHALL surface only when ≥ +10 dB stronger
...
### ADDED Requirement: A surfaced candidate SHALL include its own score and a press-`c` hint
...
### ADDED Requirement: The press-`c` action SHALL force a Wi-Fi off/on cycle
...
### ADDED Requirement: Wording SHALL match the `_health_line` and `_link_score` vocab
