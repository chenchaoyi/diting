...
Defines the contract for the Swift helper bundle (`wifiscope-helper.app`)
that owns the macOS TCC permissions wifiscope needs and exposes a
...
### ADDED Requirement: The helper SHALL ship as an `.app` bundle whose cdhash holds the user's TCC grants
The helper SHALL be installed in-tree at `helper/wifiscope-helper.app`
...
- **WHEN** the user runs `./helper/build.sh` and then `open helper/wifiscope-helper.app`
...
- **THEN** the copy has a different cdhash and requires fresh grants — wifiscope re-prompts via the helper auto-launch path

### ADDED Requirement: The helper SHALL expose discrete subcommands as the only Python integration surface
...
#### Scenario: Wi-Fi scan
- **WHEN** Python runs `wifiscope-helper wifi-scan`
...
- **WHEN** Python runs `wifiscope-helper ble-scan`
...
- **WHEN** Python runs `wifiscope-helper bluetooth-status`
- **THEN** the helper exits 0 if granted, 3 if denied/unauthorized

### ADDED Requirement: Output JSON SHALL carry an explicit `schema` integer for the wifi-scan response
...
### ADDED Requirement: The BLE scan stream SHALL emit one JSON object per advertisement
...
- **WHEN** a user runs `wifiscope-helper ble-scan | jq .`
- **THEN** every advertisement renders as a valid pretty-printed JSON object

#### Scenario: Pipe to head
- **WHEN** a user runs `wifiscope-helper ble-scan | head -n 10`
- **THEN** the helper is killed with SIGPIPE after head closes its stdin and does not hang

### ADDED Requirement: BLE advertisement objects SHALL plumb the `CBAdvertisementData` dict fields needed by downstream decoders
...
### ADDED Requirement: Connected-peripheral snapshots SHALL come from `IOBluetoothDevice.pairedDevices()`, not CoreBluetooth
...
### ADDED Requirement: The helper SHALL be auto-detectable from the Python side without configuration
The Python `_helper.find_helper()` SHALL locate the helper bundle
under `helper/wifiscope-helper.app/Contents/MacOS/wifiscope-helper`
relative to the wifiscope source root (editable install) or the
...
- **WHEN** wifiscope is installed via `uv pip install -e .`
- **THEN** `find_helper()` returns the path to the in-tree helper bundle

#### Scenario: Pip install without source
- **WHEN** wifiscope is installed via `pip install wifiscope` and the helper bundle is not present
- **THEN** `find_helper()` returns `None`, and the BLE / Wi-Fi paths fall back to direct CoreWLAN (which produces redacted scan results until the user runs `wifiscope-build-helper`)

### ADDED Requirement: The helper SHALL fail fast and loud on TCC denial
