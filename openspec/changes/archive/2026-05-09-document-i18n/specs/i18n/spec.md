...
### ADDED Requirement: The active language SHALL be resolved exactly once per process
...
### ADDED Requirement: User-facing strings SHALL go through `t()`, NOT be hardcoded
...
### ADDED Requirement: Column-aligned widgets SHALL use `pad_cells` / `fit_cells`, NEVER `str.ljust`
...
### ADDED Requirement: JSONL log keys SHALL stay English regardless of UI language
...
### ADDED Requirement: Acronyms SHALL stay English in the ZH catalog
...
### ADDED Requirement: Catalog entries SHALL preserve every `{placeholder}` from the EN source
