...
### ADDED Requirement: Each helper JSONL line SHALL produce or update exactly one `BLEDevice`
...
### ADDED Requirement: Vendor resolution SHALL run a deterministic chain of fallbacks
...
### ADDED Requirement: Connected peripherals SHALL come through a separate code path
...
### ADDED Requirement: Rotated-identifier merge SHALL fold privacy-rotated rows
...
### ADDED Requirement: The panel SHALL distinguish "(anonymous)" from "(unknown)"
...
### ADDED Requirement: BLE poller SHALL maintain RSSI smoothing for stable sort order
...
### ADDED Requirement: Schema-4 raw fields SHALL be plumbed onto `BLEDevice` for downstream decoders
...
### ADDED Requirement: BLE history SHALL be tracked per-device, capped, and pruned on snapshot churn
