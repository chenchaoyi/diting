...
### ADDED Requirement: Each scanned BSSID SHALL carry RSSI, channel, band, security mode, and BSSID itself
...
### ADDED Requirement: When the helper is unavailable, scan results SHALL be REDACTED rather than missing
...
### ADDED Requirement: Beacon IE keys (`bss_load_pct`, supports_802_11r/k/v) SHALL be optional and additive
...
### ADDED Requirement: Scan refresh SHALL respect CoreWLAN's documented throttle
...
### ADDED Requirement: BSSIDs without any scan data SHALL never appear in the Nearby list
...
### ADDED Requirement: The `current` BSSID SHALL be merged into the scan results even if scan omits it
