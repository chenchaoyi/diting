...
### ADDED Requirement: Decoders SHALL be registered functions taking a `BLEDevice` and returning a dict
...
### ADDED Requirement: Decoders SHALL never raise on malformed input
...
### ADDED Requirement: Output keys SHALL be protocol-namespaced
...
### ADDED Requirement: Bundled decoders SHALL cover the public-spec protocols seen in real environments
...
### ADDED Requirement: Decoders SHALL NOT claim semantics for bits where public docs disagree
...
### ADDED Requirement: Decoders SHALL run only when the relevant identifying bytes match
