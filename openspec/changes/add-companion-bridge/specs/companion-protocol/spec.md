...
This capability defines the canonical, versioned wire contract by which a
diting producer hands events to a companion consumer (diting-mobile) across an
untrusted store-and-forward relay. It owns the encryption envelope, the
monotonic cursor, the pairing QR payload, the relay HTTP API, the APNs trigger
shape, and the machine-readable JSON Schema + golden fixtures that downstream
consumers vendor and conform to. The event payload carried inside the envelope
is the existing pinned JSONL line schema owned by the `events` / `event-log`
capabilities; this capability references it rather than redefining it.
