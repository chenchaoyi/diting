...
This capability defines the desktop-side sender that lets a running diting
instance forward push-worthy events to a paired companion consumer. It owns the
opt-in pairing flow (QR rendered in the TUI), the git-ignored pairing state, the
event sink that taps diting's existing event fan-out, the watchdog-based
push-worthiness gate, secretbox encryption under the paired key, the relay
client, and the local offline queue. It produces envelopes that conform to the
`companion-protocol` contract; it does not redefine that wire format.
