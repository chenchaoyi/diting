...
| **v1.8.0** —— CDN 回退下载梯度：`auto`（默认）先用 `curl --max-time 20` 试 GitHub，失败时回退到 `https://ghproxy.com/<github-url>`；`github` 锁回退（与改造前一致）；`ghproxy` 跳过 GitHub 直接走镜像（适合 CN 已知不通的用户）；`DITING_INSTALL_MIRROR` 取非法值时下载前即 abort。SHA256 校验对实际下载到的字节做，信任仍锚定 canonical SHASUMS（与哪条 URL 服务了字节无关） | `test_install.py::test_mirror_env_default_auto_ladder`、`::test_mirror_env_invalid_value_aborts`、`::test_mirror_env_github_only_skips_ghproxy_path`、`::test_mirror_env_ghproxy_only_skips_github_path`、`::test_auto_ladder_falls_back_when_github_fails`、`::test_auto_ladder_emits_no_notice_when_github_succeeds`、`::test_sha_verification_runs_against_ghproxy_served_bytes`、`::test_completion_notice_uses_zh_locale_when_helper_lang_is_zh` |
