...
| **agent-friendly-cli** —— `--json`：`analyze` 输出单个纯 JSON 文档（`report_to_dict`，键即使在 `--lang zh` 下也保持英文）；`once` 输出 `{backend,permission_state,associated,connection}`；`watch` 每事件一个对象（`_watch_event_to_json`，去重）；`connection_to_dict` 往返 | `test_cli.py::test_analyze_json_is_pure_parseable_document`、`::test_analyze_json_keys_stay_english_under_zh`、`::test_watch_event_to_json_shapes_each_kind`、`::test_connection_to_dict_round_trips`、`test_analyze.py::test_report_to_dict_is_json_serializable_with_stable_keys` |
| **agent-friendly-cli** —— 每个子命令接受 `--help`，带 EXAMPLES + `--json` 说明；CLI help 按设计为英文专用（运行时文案 / 报错仍双语） | `test_cli.py::test_subcommand_help_prints_and_exits_zero` |
