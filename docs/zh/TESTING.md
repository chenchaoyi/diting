...
| 分类器：HomePod（`AirPlay` + `AirPlay audio`）→ speaker；iPad（`AirPlay` + `Apple Companion`）→ phone；Apple TV（仅 AirPlay）→ tv。**针匹配 mdns 模块在 LANHost 上存的人类可读类别字符串（`AirPlay`、`AirPlay audio`、`Apple Companion`、`HomeKit`、`Sonos` 等），不是原始 service-type 名（`_raop._tcp`、`_companion-link._tcp` 在 `bonjour_services` 里永远不会出现）。** | `test_device_class.py::test_homepod_airplay_audio_signals_speaker_not_tv`、`::test_homepod_full_apple_signature_signals_speaker_not_phone`、`::test_ipad_airplay_plus_companion_signals_phone_not_tv`、`::test_apple_tv_airplay_alone_still_signals_tv` |
