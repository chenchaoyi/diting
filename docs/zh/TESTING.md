...
| Bonjour TXT 里的 Apple 型号代码（`Mac14,2`、`AudioAccessory6,1`、`iPhone16,1`、`AppleTV14,1`）通过 `_APPLE_MODEL_PREFIXES` 映射到设备类。最高置信度的 Apple 侧信号 —— 优先于规则表；解决 Bonjour 类别区分不开 Mac vs HomePod 的根本难题 | `test_device_class.py::test_apple_model_mac_signals_laptop`、`::test_apple_model_audioaccessory_signals_speaker`、`::test_apple_model_iphone_signals_phone`、`::test_apple_model_appletv_signals_tv`、`::test_apple_model_unknown_prefix_falls_through`、`::test_apple_model_macbookpro_explicit_prefix_wins` |
