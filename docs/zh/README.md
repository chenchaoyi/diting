...
善恶、识真伪、知前世今生。项目取这个名字也正是这层意思 ——
你的 Mac 本就「听见」了周围的无线世界（Wi-Fi 来来去去、BLE
设备在身旁轻声广播、上游链路在悄悄丢包），而谛听要做的，就
是代它听，再老老实实把听见的告诉你。

**天耳**（tianer）是佛家「六神通」之一的「天耳通」—— 谛听
本身所具备的那种神通，能闻凡夫之耳所不能闻：太远、太轻、
太隐蔽的声音都听得见。在本项目里，这个名字属于 Swift 辅助
进程包（`diting-tianer.app`）：它持有 macOS 的 TCC 授权 ——
定位服务（macOS 14.4+ 上 Wi-Fi 扫描的 SSID / BSSID 才不会被
脱敏成空）与蓝牙（BLE 广告数据才可见）—— 并把原始信号转给
Python 写的 TUI。Python 前端负责聆听、转述、排序；*tianer*
才是那只被操作系统真正允许去听的耳朵。
...
| `j` | （在 Wi-Fi 详情页内）加入当前查看的 SSID —— 已存 Keychain 的网络无感加入，新网络弹出原生 macOS 密码框。**不是无损切换**：跨 SSID 会断开当前连接 ~2–5 秒。企业 / 802.1X 网络会被拒绝并提示。 |
