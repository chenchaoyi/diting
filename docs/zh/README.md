...
*该不该换*、*OS 什么时候漫游过（或没漫游）*。`diting` 把这个黑盒子拆开，
做成一个 TUI：
...
- **JSONL 会话日志 + 重放模式** —— `diting log session.jsonl`
  持续追加连接 / 扫描 / 漫游 / 延迟事件；`diting replay <file>`
