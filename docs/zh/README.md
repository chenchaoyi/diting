...
项目里的分工正是同一套结构。Python 写的 TUI 扮演谛听 ——
负责聆听、转述、排序 —— 但在 macOS 14.4+ 上它一个人听不见
多少：未授权扫描取回的 SSID 是 `None`、BSSID 被脱敏，蓝牙
更是被 TCC 整个圈在外面。Swift 写的辅助进程包
（`diting-tianer.app`）才是持有能撩开这层帷幕的 macOS 授权的
角色 —— 定位服务让 Wi-Fi 扫描结果不再脱敏、蓝牙让 BLE 广告
可见 —— 再把原始信号转交给前端。helper 才是那只耳朵；Python
前端是听懂它所听之物的那一位。
