...
国内网络？installer 会在 GitHub 直连下载卡住时（`curl --max-time 20`）
自动回退到公开镜像 [`ghproxy.com`](https://ghproxy.com)。SHA256 校验
仍然锚定 canonical `SHASUMS256.txt`，所以恶意镜像会被哈希不一致拦下来。
如果你已经知道 GitHub 在你的网络上完全不通，可以跳过 20 秒的 GitHub
首次尝试：
...
`DITING_INSTALL_MIRROR=github` 锁回退（不走镜像，与 v1.8.0 之前的行为
一致），适合需要单一信任路径的场景。
